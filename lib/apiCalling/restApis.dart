  ...
  Future<ModelHomePage> login(String empdetails) async {
    ...
    try {
      var url = Uri(
          ...
          path:
              'WebServices/TCIL_SuppApp_WebServices_Demo/SuppAppService.svc/getSuppAppOTP');

      var url1 = Uri.decodeComponent(url.toString());

      return _netUtil.post(url1, body: empdetails).then((dynamic res) {
...
//        }
  ...
  Future<List> currencyList() async {
    try {
      var url = Uri(
          scheme: 'https',
          host: 'api.coingecko.com',
...
//        }
  ...
  Future<ModelMarketChart> marketChart(
      String coinID, String currency, String noOfDays) async {
    try {
      var url = Uri(
          scheme: 'https',
          host: 'api.coingecko.com',
